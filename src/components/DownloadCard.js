...
import ToastDownloadSuccess from "./ToastDownloadSuccess";
      ...
      ws.onmessage = (ev) => {
        ...
        if (d.status === "downloading") {
          ...
          dispatch({
            type: "UPDATE_ITEM",
            id: item.id,
            updates: { progress: pct, speed: d.speed, eta: d.eta },
          });
        }

        else if (d.status === "finished_file") {
          const path = fileUrl(d.filename);

          dispatch({
            type: "UPDATE_ITEM",
            id: item.id,
            updates: { status: "finished", filename: d.filename, progress: 100 },
          });
          ...
          toast.custom(
            (t) => (
              <ToastDownloadSuccess
                filename={d.filename}
                fileUrl={path}
                folderPath="Downloads" // you can wire backend support later
  ...
  const start = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.REACT_APP_BACKEND || "http://127.0.0.1:8000"}/download`,
        {
          method: "POST",
          ...
          body: JSON.stringify({
            url: item.url,
            id: item.id,
            mode: item.mode.startsWith("mp") ? "audio" : "video",
          }),
        }
      );
      ...
      <div className="dc-body">
        <div className="dc-row">
          ...
          <div className="dc-actions">
            {item.status !== "running" && item.status !== "finished" && (
              <button className="mini" onClick={start}>
                Start
              </button>
            )}
        ...
        <div className="dc-footer">
          ...
          <div className="small-muted">
            {item.mode} •{" "}
            {item.speed ? `${(item.speed / 1024).toFixed(1)} KB/s` : ""}
            {" • "}ETA: {item.eta ?? "--"}
          </div>
        </div>
      </div>
